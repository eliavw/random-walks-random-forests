...
from .extra import debug_print
VERBOSITY = 0


# Main
def run_script(script_fname,
               config_fname,
               log_fname,
               fold=None,
               timeout=None,
    ...
    type(Fold):         {}
    type(Timeout):      {}
    """.format(fold, timeout, type(fold), type(timeout))
    debug_print(msg, V=VERBOSITY)

    cmd = generate_command(script_fname, config_fname, fold=fold, q_idx=q_idx)  # Build command
    mon = generate_monitor(log_fname, timeout)

    return run_process(cmd, mon)


def generate_command(script_fname, config_fname, script_prefix="python", fold=None, q_idx=None):
    """
