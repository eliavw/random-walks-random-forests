...
from ..utils.extra import debug_print, generate_keychain
from ..utils.filesystem import (
    collect_fnames_from_folder,
    ensure_dir,
    gen_derived_fnames,
    make_fname,
)
...
class RunModulo(Runner):
    def __init__(self, **kwargs):

        init_kwargs = filter_kwargs_function(super(RunModulo, self).__init__, **kwargs)
        super(RunModulo, self).__init__(script="run_Modulo", **init_kwargs)
    ...
    def default_config(self, **kwargs):
        ...
        self.config["child"] = kwargs.get("child", "Mercs")
    ...
    def update_config(self, **kwargs):
        ...
        for kind in {child, "eval"}:
            super(RunModulo, self).update_config(
                kind=kind, hierarchical=False, overwrite=False, **kwargs
            )
            kwargs = {
                k: v for k, v in kwargs.items() if kind not in k
            }  # Exclude kwargs passed on this way

        # Update config
        super(RunModulo, self).update_config(hierarchical=True, **kwargs)

        # Special cases - query (qry)
        qry_kwargs = self._generate_qry_params()
        super(RunModulo, self).update_config(hierarchical=True, **qry_kwargs)

        # Special cases - model (mod)
        mod_kwargs = self._generate_mod_params()
        super(RunModulo, self).update_config(hierarchical=True, **mod_kwargs)
    ...
    def save_config(self, overwrite=True):
        ...
        super(RunModulo, self).save_config()
        return
    ...
    def generate_commands(self, **kwargs):
        ...
        head_tuple = ("script_fname", "config_fname", "log_fname", "fold", "timeout")
        data_tuple = ("script_fname", "config_fname", "log_fname", 0, 60)
        tuple_list = [data_tuple] * len(folds)
        for f_idx, fold in enumerate(folds):
            log_fname = [t[1] for t in log_fnames if t[0] == fold][0]

            param_tuple = (script_fname, config_fname, log_fname, fold, timeout)
    ...
    def generate_outp_fns(self):
        ...
        data_tuple = (0, 0, "str", "str", "str", "str")
        tuple_list = [data_tuple] * len(folds)
    ...
    def default_io_config(self, root_dir=None, dataset=None, idx=None):
        ...
        # io-dirs
        d = super(RunModulo, self).default_io_config_dirs(
            root_dir=root_dir, dataset=dataset, idx=idx
        )
    ...
    def _default_io_config_file(self, dirs=None):
        ...
        # Commands (cmd)
        d["script"] = make_fname(name=self.script, extension="py", dname=dirs["cmd"])

        # Resources (resc)
        d["train_data"] = collect_fnames_from_folder(
            dirs["resc-data-ds"], criteria=["Train"], indexed=True
        )
        d["test_data"] = collect_fnames_from_folder(
            dirs["resc-data-ds"], criteria=["Test"], indexed=True
        )
        ...
        if not (len(d["train_data"]) == len(d["test_data"]) > 0):
            ...
            folds: {}
            """.format(
                d["train_data"], d["test_data"], folds
            )
        ...
        d["mod-config"] = gen_derived_fnames(
            d["train_data"],
            name=child,
            extension="pkl",
    ...
    def _generate_qry_params(self):
        ...
        qry_codes_fname = collect_fnames_from_folder(qry_codes_dir, criteria=[qry_key])
        ...
        qry_config_fname = collect_fnames_from_folder(
            qry_config_dir, criteria=[qry_key]
        )

        key = generate_keychain(["qry", "config"])
        parameters = {**parameters, "io.file." + key: qry_config_fname[0]}
    ...
    def _generate_mod_params(self):
        ...
        if mod_config is not None:
            mod_type = self.config["mod"].get("type", "Mercs")
            ...
            mod_fnames = collect_fnames_from_folder(
                mod_dir, criteria=criteria, indexed=True
            )
            ...
            mod_config_fname = collect_fnames_from_folder(
                mod_config_dir, criteria=criteria
            )

            key = generate_keychain(["load", "mod", "config"])
            parameters = {**parameters, "io.file." + key: mod_config_fname[0]}
        else:
            parameters = {}
