...
class RunPxL(Runner):
    ...
    def __init__(self, **kwargs):
        ...
        init_kwargs = filter_kwargs_function(super(RunPxL, self).__init__,
                                             **kwargs)
        super(RunPxL, self).__init__(script='run_PxL', **init_kwargs)

        # Folder structure of products.
    ...
    def update_config(self, **kwargs):
        ...
        pass_on_categories = {child, "eval"}

        # Configs that we just pass on (i.e., do not unpack these!)
        for kind in pass_on_categories:
            super(RunPxL, self).update_config(kind=kind,
                                              hierarchical=False,
                                              overwrite=False,
                                              **kwargs)
        ...
        # Update config RunPxL
        super(RunPxL, self).update_config(hierarchical=True,
                                          **kwargs)
    ...
    def default_io_config(self, root_dir=None, dataset=None, idx=None):
        ...
        # io-dirs
        d = super(RunPxL, self).default_io_config_dirs(root_dir=root_dir,
                                                       dataset=dataset,
                                                       idx=idx)
    ...
    def _generate_mod_params(self):
        """
        This is only relevant when a model needs to be loaded.
        ...
        if mod_config is not None:
            mod_type = self.config['mod'].get('type', 'PxL')
