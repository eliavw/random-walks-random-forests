...
class RunPxSParallel(RunPxS):
    ...
    def __init__(self, **kwargs):
        ...
        init_kwargs = filter_kwargs_function(super(RunPxSParallel, self).__init__,
                                             **kwargs)
        super(RunPxSParallel, self).__init__(script='run_PxS_parallel', **init_kwargs)

        self.prods = {'config', 'logs', 'results', 'timings', 'tmp'}  # What this produces
    ...
    def default_config(self, **kwargs):
        ...
        self.config['child'] = kwargs.get('child', 'PxS')
    ...
    def update_config(self, **kwargs):
        ...
        for kind in {child, 'eval'}:
            super(RunPxSParallel, self).update_config(kind=kind,
                                                      hierarchical=False,
                                                      overwrite=False,
                                                      **kwargs)
        ...
        # Update config RunPxSParallel
        super(RunPxSParallel, self).update_config(hierarchical=True,
                                                  **kwargs)
    ...
    def save_config(self, overwrite=True):
        ...
        super(RunPxSParallel, self).save_config()
        return
        ...
        for f_idx, fold in enumerate(folds):
            for q_idx in range(nb_queries):
                ...
                param_tuple = (script_fname,
                               ...
                               fold,
                               timeout)
                idx = f_idx*nb_queries+q_idx
                tuple_list[idx] = param_tuple
    ...
    def default_io_config(self, root_dir=None, dataset=None, idx=None):
        ...
        # io-dirs
        d = super(RunPxSParallel, self).default_io_config_dirs(root_dir=root_dir,
                                                               dataset=dataset,
                                                               idx=idx)
    ...
    def _generate_mod_params(self):
        ...
        if mod_config is not None:
            mod_type = self.config['mod'].get('type', 'PxS')
